...
(function () {
  ...
  // soloSub = 'art:painting' 之类:只看这一个小类(双击子类触发),优先级高于领域/子类开关
  const itemOn = it => S.soloSub
    ? (it.d + ':' + (it.sub || '')) === S.soloSub
    : S.enabled.has(it.d) && !(it.sub && S.subOff.has(it.d + ':' + it.sub));
  ...
  function buildLegend() {
    ...
    const subBtns = [];  // 子类按钮池 [key, btn]
    let soloId = null;   // 记住当前独显领域(不能看 enabled 推断:双击前必经的单击会把状态搅乱)
    const syncAllBtns = () => {
      domBtns.forEach(([bid, btn]) => btn.setAttribute('aria-pressed', S.enabled.has(bid)));
      subBtns.forEach(([k, btn]) => btn.setAttribute('aria-pressed', !S.subOff.has(k)));
    };
    const exitSoloSub = () => { // 退出子类独显 → 全部恢复
      if (!S.soloSub) return;
      S.soloSub = null;
      S.enabled.clear(); DATA.domains.forEach(x => S.enabled.add(x.id));
      S.subOff.clear();
      syncAllBtns();
    };
    const soloDomain = id => {
    ...
    const soloSub = (key, did) => { // 双击子类:只看这一个小类;再双击恢复全部(单击已延迟化,活状态判定可靠)
      if (S.soloSub === key) { exitSoloSub(); }
      else {
        S.soloSub = key; soloId = null;
        domBtns.forEach(([bid, btn]) => btn.setAttribute('aria-pressed', bid === did));
        subBtns.forEach(([k, btn]) => btn.setAttribute('aria-pressed', k === key));
      }
      if (GLOBE3D) GlobeView.setEnabled();
      dismissHint();
    };
    DATA.domains.forEach(d => {
      ...
      b.onclick = () => {
        ...
        lastTap = now;
        exitSoloSub(); // 子类独显中来任何单击 → 先恢复再执行开关
      ...
      if (subs.length) {
        ...
        subs.forEach(([slug, name]) => {
          ...
          c.title = '单击:开/关 · 双击:只看「' + name + '」';
          c.innerHTML = '<span class="dot"></span>' + name + '<span class="ct">' + subCounts[key] + '</span>';
          let subTimer = null;
          c.onclick = () => {
            // 单击延迟 300ms 执行;窗口内第二击=双击独显,并取消单击(否则单击会先清掉独显态,恢复判定必错)
            if (subTimer) { clearTimeout(subTimer); subTimer = null; soloSub(key, d.id); return; }
            subTimer = setTimeout(() => {
              subTimer = null;
              exitSoloSub();
